 ...
 * Narrowing 종류
 *
 * 1) Assingnment Narrowing
 * 2) typeof Narrowing
 * 3) Trythiness Narrowing
...
let stringOrBool2: string | boolean = Math.random() > 0.5 ? "1123" : true;
...
let numberOrStingOrNull: number | string | null =
  Math.random() > 0.5 ? 1123 : Math.random() > 0.5 ? "안유진" : null;

if (typeof numberOrStingOrNull === "number") {
  numberOrStingOrNull;
} else {
  numberOrStingOrNull;
}
